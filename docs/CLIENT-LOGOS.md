...
bottom of `html/clientele.html` marked
...
```
python3 tools/normalize-client-logos.py  <folder-of-new-logos>  images/clients
```
...
**Speed.** One shared setting, now in `css/pages/clientele.css` next to the rest of the
design tokens:
...
**Row count.** In `js/pages/clientele.js`, `var ROWS = 5`, paired with the five
`data-clx-rail` elements in `html/clientele.html`. Rows alternate direction and
