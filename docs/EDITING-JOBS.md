...
> `server/careers-jobs.php`. Manage live openings in Ceipal; see
> [CEIPAL-INTEGRATION.md](CEIPAL-INTEGRATION.md) for deployment and troubleshooting.
> The instructions below apply only to the optional manual `data/jobs.json` feed.
> To use that feed instead, explicitly set `DATA_URL` in `js/pages/careers.js` to `../data/jobs.json`.

You only ever edit **one file: `data/jobs.json`**.

You never need to touch the design, the layout, the colours, or any HTML.
The Careers page reads `data/jobs.json` every time someone loads it and builds the
...
- Open `data/jobs.json` in **Notepad** (Windows), **TextEdit** (Mac), or **VS Code**.
...
- `js/pages/careers.js` renders the board. `PAGE_SIZE` at the top of that file controls
  how many rows show before the **View All Openings** button appears (currently 8).
- The loader accepts either `{ "jobs": [ ... ] }` or a bare `[ ... ]` array.
- Any opening can be linked to directly: `html/career.html#job-senior-power-bi-developer`
...
- `data/jobs.json` is fetched with `cache: "no-store"`, so edits go live without a
  cache purge.
- **This needs the site to be served over http/https.** Opening `html/career.html`
...
- To move to WordPress or a CMS later, point `DATA_URL` in `js/pages/careers.js` at an
